...
calc_intY1=0.7*(rand_intX-1)+2
dotcolor=(rand_intY<calc_intY1)
...
class AreaUnderLine(MovingCameraScene):
    def construct(self):
        ...
        p1Label=Tex("$P_1$").shift(DOWN*0.5)
        p2Label=Tex("$P_2$").shift(1.2*LEFT,UP*0.9)
        ...
        image=VGroup(ax,labels,squareDots,square,braceA,braceB,braceAtext,braceBtext,PLabels)
